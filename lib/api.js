...
const analyzeFiles = function (room) {
	...
	return executeCommandLineProcess(cmd2, rejectEmpty).then(
		(data) => {
			...
			return runCausalPathForAll(dirs, 0).then(() => {
				let cmd3 =
					'find ./analysisOut/' + room + ' -name "causative.json"'; //now find the analysis files
				return executeCommandLineProcess(cmd3, rejectEmpty).then(
					(data2) => {
						...
						console.log(cmd3);
						console.log(
							'In analysis zip found causative.json as ' + data2
						);
						return data2;
					},
					(error) => {
...
http.post('/calculateDemoFolderFilePaths', function (req, res) {
	...
	const filePaths = [];
	const fillFilePaths = function (dirPath) {
...
http.post('/downloadRequest', function (req, res) {
	...
	try {
		...
		executeCommandLineProcess(
			'zip -r ./analysisOut/' + room + ' ./analysisOut/' + room
		).then(function (data) {
			fs.readFile(
				'./analysisOut/' + room + '.zip',
	...
	try {
		...
		let p1 = new Promise(function (resolve, reject) {
			for (var i = 0; i < inputFiles.length; i++) {
				(function (file) {
					fs.writeFile(
						'./analysisOut/' + room + '/' + file.name,
						file.content,
						function (err) {
							if (err) {
								console.log(err);
								reject(
									'Error while writing content of the file ' +
										file.name
								);
							}
							written++;
