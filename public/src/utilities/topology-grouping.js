...
function groupTopology(cyElements) {
    ...
    nodes.forEach(function (node) {
        ...
        edges.forEach(function (edge) {
            if (edge.data.source == node.data.id) node.outgoing.push(edge);
            if (edge.data.target == node.data.id) node.incoming.push(edge);
        });
    });
    ...
    for (var i = 0; i < nodes.length; i++) {
        for (var j = i + 1; j < nodes.length; j++) {
            if (areNodesAnalogous(nodes[i], nodes[j], edges)) {
                //combine analogous nodes under the same parent
                if (nodes[i].data.parent != null) {
                    //first node already in the newNode list
                    ...
                    //Create a new node and assign it as the parent of both nodes
                    nodes[i].data.parent = nodes[j].data.parent = "p" + parentId;
                    parentId++;
                    newNodes.push({
                        data: {
                            id: nodes[i].data.parent,
                            children: [nodes[i].data.id, nodes[j].data.id],
                            edgeType: findCliqueEdgeType(
                                nodes[i].data.id,
                                nodes[j].data.id,
                                edges
                            ),
                        },
                        incoming: nodes[i].incoming,
                        outgoing: nodes[i].outgoing,
                    });
                }
            }
        }
    ...
    edges.forEach(function (edge) {
        let newEdge;
        if (nodeHash[edge.data.source].data.parent != null) {
            //change the source
            ...
            if (nodeHash[edge.data.target].data.parent != null) {
                ...
                if (newSource !== target)
                    newEdge = {
                        data: {
                            id: newSource + "_" + edge.data.target,
                            source: newSource,
                            target: target,
                            edgeType: edge.data.edgeType,
                        },
            ...
            if (nodeHash[edge.data.target].data.parent != null) {
                ...
                if (source !== newTarget)
                    newEdge = {
                        data: {
                            id: edge.data.source + "_" + newTarget,
                            source: source,
                            target: newTarget,
                            edgeType: edge.data.edgeType,
                        },
                ...
                if (source !== target)
                    newEdge = {
                        data: {
                            id: edge.data.source + "_" + edge.data.target,
                            source: source,
                            target: target,
                            edgeType: edge.data.edgeType,
                        },
        ...
        if (newEdge) {
            if (edge.data.pcLinks) newEdge.data.pcLinks = edge.data.pcLinks;
            else newEdge.data.pcLinks = [];
            ...
            if (!exists) newEdges.push(newEdge);
        }
    });
...
function areNodesAnalogous(n1, n2, edges) {
    if (
        n1.incoming.length != n2.incoming.length ||
        n1.outgoing.length != n2.outgoing.length
    )
        return false;

    if (
        !noEdgeBetween(n1.data.id, n2.data.id, edges) &&
        !isCliqueBetween(n1.data.id, n2.data.id, edges)
    )
        return false;
    ...
    for (var i = 0; i < n1.incoming.length; i++) {
        ...
        if (isEdgeBetween(n1.data.id, n2.data.id, edge1))
            //skip edges between n1 and n2
            continue;
        ...
        for (var j = 0; j < n2.incoming.length; j++) {
            ...
            if (
                edge1.data.edgeType == edge2.data.edgeType &&
                edge1.data.source == edge2.data.source
            ) {
                exists = true;
                break;
            }
...
function isEdgeBetween(id1, id2, edge) {
    if (edge.data.source == id1 && edge.data.target == id2) return true;
    if (edge.data.source == id2 && edge.data.target == id1) return true;
    return false;
}
...
function noEdgeBetween(id1, id2, edges) {
    edges.forEach(function (edge) {
        if (edge.data.source == id1 && edge.data.target == id2) return false;
        if (edge.data.target == id1 && edge.data.source == id2) return false;
    });
...
function isCliqueBetween(id1, id2, edges) {
    ...
    var eId2 = null;
    edges.forEach(function (edge) {
        if (edge.data.source == id1 && edge.data.target == id2) eId1 = edge.data.id;
        else if (edge.data.target == id1 && edge.data.source == id2)
            eId2 = edge.data.id;
    });

    if (eId1 != null && eId2 != null) return true;
...
function findCliqueEdgeType(id1, id2, edges) {
    for (var i = 0; i < edges.length; i++) {
        //careful! does not work with iterators
        var edge = edges[i];
