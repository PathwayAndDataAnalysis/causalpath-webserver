    ...
    return {
        ...
        clearModel: function () {
            if (model.get("_page.doc.cy")) {
                if (model.get("_page.doc.cy.nodes")) model.del("_page.doc.cy.nodes");
                if (model.get("_page.doc.cy.edges")) model.del("_page.doc.cy.edges");
        ...
        initModelFromJson: function (jsonObj) {
            ...
            if (jsonObj.edges) {
                ...
                for (var i = 0; i < jsonObj.edges.length; i++) {
                    ...
                    if (!edgeId) edgeId = edge.data.source + "-" + edge.data.target;
        ...
        initModelNodePositions: function (nodes) {
            for (var i = 0; i < nodes.length; i++) {
                model.set(
                    "_page.doc.cy.nodes." + nodes[i].id() + ".position",
                    nodes[i].position()
                );
            }
        },
        ...
        getModelParameter: function (ind) {
            ...
            if (ind >= 0 && ind < parameterList.length) return parameterList[ind];
            else {
        ...
        findModelParameterFromId: function (id) {
            ...
            for (let i = 0; i < parameterList.length; i++) {
                if (parameterList[i].ID === id) return parameterList[i];
            }
        ...
        setModelParameterValue: function (ind, cnt, entryInd, val) {
            model.set(
                "_page.doc.parameters." + ind + ".value." + cnt + "." + entryInd,
                val
            );
        },
            ...
            for (let i = 0; i < parameterList.length; i++) {
                ...
                if (model.get("_page.doc.parameters." + i + ".cnt") == null) {
                    ...
                    for (let j = 0; j < param.EntryType.length; j++)
                        model.set(
                            "_page.doc.parameters." + i + ".domId.0." + j,
                            param.ID + "-0-" + j
                        ); //for multiple fields

                    model.set(
                        "_page.doc.parameters." + i + ".batchDomId",
                        param.ID + "-batch"
                    ); //for batch values
                    model.set(
                        "_page.doc.parameters." + i + ".batchModalDomId",
                        param.ID + "-batchModal"
                    ); //for batch values
                }
        ...
        resetToDefaultModelParameters() {
            ...
            for (let i = 0; i < parameterList.length; i++) {
                model.set(
                    "_page.doc.parameters." + i + ".value",
                    parameterList[i].Default
                );
            }
        },
