    ...
    nodesData.forEach(function (nodeData) {
        ...
        sites.forEach(function (site, i) {
            if (i % 2 == 0) {
                ...
                //Draw sites at the bottom of the node
                x = l + d * (i - 1);
                y = 0.5;
            }
...
function finilizeLayoutOptions(layoutOptions) {
    ...
    let extendedOptions = _.extend({}, layoutOptions, {
        randomize,
        tilingCompareBy,
    });
 ...
 */
module.exports.convertModelJsonToCyElements = function (
    modelCy,
    doTopologyGrouping
) {
    var nodes = [];
    ...
    if (doTopologyGrouping) return groupTopology(cyElements);
    else return cyElements;
};
...
module.exports.createContainer = function (
    ...
    var cyElements = module.exports.convertModelJsonToCyElements(
        modelCy,
        doTopologyGrouping
    );
    ...
    cytoscape({
        ...
        ready: function () {
                ...
                menuItems: [
                    {
                        ...
                        onClickFunction: function (event) {
                            ...
                            if (links && links.length > 0) {
                                uriStr = links[0];
                                for (let i = 1; i < links.length; i++) uriStr += "," + links[i];
            ...
            cy.on("unselect", "node", function (e) {
                ...
                var backgroundColor = modelManager.getModelNodeAttribute(
                    this.id(),
                    "css.backgroundColor"
                );
            ...
            cy.on("tap", "node, edge", function (event) {
                ...
                var destroyTippy;
                var relatedNodes = ele.isNode()
                    ? ele
                    : ele.source().union(ele.target());
